      ...
      <q-card flat bordered>
        <q-card-section class="q-pa-xs">
          ...
          <q-markup-table dense class="styled-table">
            ...
            <tbody>
            <tr v-for="(productoladrillo, index) in productoladrillos" :key="index">
              <td>
                <q-btn-dropdown
                  size="10px"
                  color="primary"
                  ...
                  dense
                  class="q-mr-sm">
                  <q-item clickable v-ripple @click="showProducto(productoladrillo)" v-close-popup>
                    <q-item-section>Editar</q-item-section>
                  </q-item>
                </q-btn-dropdown>
              </td>
  
              <td>{{ productoladrillo.nombre_pro }}</td>
              <td>{{ productoladrillo.precio_pro }}</td>
              <td>{{ productoladrillo.precio_compra }}</td>
              <td>{{ productoladrillo.cantidad_pro }}</td>
              <td>{{ productoladrillo.descripcion_pro }}</td>
              <td>{{ productoladrillo.tonelada }}</td>
              <td>
                <a v-if="productoladrillo.foto_pro" :href="$url+'..'+productoladrillo.foto_pro" target="_blank">
                  <!--                                  ver foto-->
                  <q-img :src="$url+'..'+productoladrillo.foto_pro" alt="Imagen del producto" class="img-thumbnail" height="10"/>
                </a>
              </td>
  
              <td>{{ productoladrillo.estado_pro }}</td>
              <td>{{ productoladrillo.id }}</td>
              <td>{{ productoladrillo.categoria_pro }}</td>
              <td>{{ productoladrillo.marca_pro }}</td>
            </tr>
            </tbody>
          </q-markup-table>
  <!--        <pre>{{productos}}</pre>-->
    ...
    <q-dialog v-model="dialog" :position="esMovil ? undefined : 'right'" :maximized="true" transition-show="slide-left"
      ...
      <q-card style="width: 450px; max-width: 100vw;">
        <q-card-section class="row items-center q-px-md bg-primary text-white q-px-none">
          ...
          <q-space/>
          <div class="text-h6">{{ productoladrillo.id ? 'Editar' : 'Nuevo' }} Producto ladrillo</div>
        </q-card-section>
        <q-card-section>
          <q-form @submit.prevent="confirmarGuardar">
            <div class="row">
              ...
              <div class="col-12">
                <q-input
                  dense
                  v-model="productoladrillo.marca_pro"
                  outlined
                  label="Marca"
                  :rules="[val => !!val || 'Este campo es requerido']"
                  @input="productoladrillo.marca_pro = productoladrillo.marca_pro.toUpperCase()"
                  style="text-transform: uppercase;"/>
              </div>
              <div class="col-12">
                <q-input
                  dense
                  v-model="productoladrillo.nombre_pro"
                  outlined
                  label="Nombre Producto"
                  :rules="[val => !!val || 'Este campo es requerido']"
                  @input="productoladrillo.nombre_pro = productoladrillo.nombre_pro.toUpperCase()"
                  style="text-transform: uppercase;"/>
              </div>
              <div class="col-12">
                <q-input
                  dense
                  v-model="productoladrillo.descripcion_pro"
                  outlined
                  label="Descripcion Producto"
                  @input="productoladrillo.descripcion_pro = productoladrillo.descripcion_pro.toUpperCase()"
                  hint=""
                  style="text-transform: uppercase;"/>
              </div>
              <div class="col-6">
                <q-input dense v-model="productoladrillo.precio_pro" outlined label="Precio Venta" type="number"
                         hint=""
                />
              </div>
              <div class="col-6">
                <q-input dense v-model="productoladrillo.precio_compra" outlined label="Precio Compra" type="number"
                         hint=""
                />
              </div>
              <div class="col-12">
                <q-input dense v-model="productoladrillo.cantidad_pro" outlined label="Cantidad en stok" type="number"
                         hint=""
                >
              ...
              <div class="col-12">
                <q-input dense v-model="productoladrillo.tonelada" outlined label="Cantidad Mayor general en ventas" type="number"
                         hint=""
                >
                </q-input>
              </div>
              ...
              <div class="d-grid col-6 mx-auto mb-3">
                <img v-if="productoladrillo.foto_pro"
                     :src="productoladrillo.foto_pro.includes('data') ? productoladrillo.foto_pro : $url + '..' + productoladrillo.foto_pro"
              ...
              <div class="col-12">
                <q-select
                  dense
                  v-model="productoladrillo.estado_pro"
  ...
  export default {
    ...
    methods: {
      aumentarPorTonelada() {
        this.$q.dialog({
          ...
          const tonelada = parseInt(this.productoladrillo.tonelada);
          this.productoladrillo.cantidad_pro = (parseInt(data) * tonelada) + parseInt(this.productoladrillo.cantidad_pro);
        });
      },
      exportExcel() {
          let data = [{
            sheet: "Productoladrillos",
            columns: [
            ...
            content: this.productoladrillos
          }]
      ...
      showProducto(productoladrillo) {
        this.productoladrillo = {...productoladrillo}; // Esto es importante para asegurarse de que los datos del producto se copien correctamente
      ...
      submit() { 
        ...
        if (this.productoladrillo.id) {
          ...
          this.$axios.post('productoladrillos', this.productoladrillo)
            .then(response => {
              this.productoladrillos.push(response.data);
              this.dialog = false;
            })
            .catch(error => {
              console.log(error);
      ...
      getProductos() {
    this.$axios.get('productoladrillos')
      .then(response => {
        this.productoladrillos = response.data;
  
        // Verificar si algún producto tiene cantidad_pro / tonelada < 2
        const sinInventario = this.productoladrillos.some(productoladrillo => {
    ...
    productoladrillo.cantidad_pro / productoladrillo.tonelada < 2
      ...
      if (productosAgotados.length > 0) {
        const nombres = productosAgotados.map((productoladrillo) => productoladrillo.nombre_pro).join(", ");
        this.$q.dialog({
